const SAMPLE_RUNS = [
  { rider: "MaloFoil", category: "Foil", time: 3842, recordedAt: "2026-08-30T15:22:00Z" },
  { rider: "Nico83", category: "Foil", time: 4017, recordedAt: "2026-08-31T10:13:00Z" },
  { rider: "MaloFoil", category: "Foil", time: 3768, recordedAt: "2026-09-01T14:18:00Z" },
  { rider: "LinaRide", category: "Wake", time: 4241, recordedAt: "2026-08-29T09:04:00Z" },
  { rider: "TomAir", category: "Wake", time: 3995, recordedAt: "2026-09-01T17:42:00Z" },
  { rider: "Nico83", category: "Foil", time: 3926, recordedAt: "2026-09-01T18:01:00Z" },
  { rider: "CamFoil", category: "Foil", time: 4410, recordedAt: "2026-08-28T11:32:00Z" }
];
...
function buildRanking(runs) {
  ...
  runs.forEach((run) => {
    const key = `${run.rider.toLocaleLowerCase("fr")}:${run.category}`;
    const current = riders.get(key) || { rider: run.rider, category: run.category, runs: 0, best: Infinity };
    current.runs += 1;
...
function renderRanking() {
  ...
  const ranking = buildRanking(readRuns()).filter((entry) => {
    const categoryMatches = selectedCategory === "Tous" || entry.category === selectedCategory;
    return categoryMatches && entry.rider.toLocaleLowerCase("fr").includes(query);
  });
  ...
  rankingList.replaceChildren(...ranking.map((entry, index) => {
    ...
    item.innerHTML = `
      ...
      <span class="rider">
        ...
        <span><span class="rider-name"></span><span class="rider-category">${entry.category}</span></span>
      </span>
...
runForm.addEventListener("submit", (event) => {
  ...
  const runs = readRuns();
  runs.push({ rider, category, time: seconds * 100 + centiseconds, recordedAt: new Date().toISOString() });
  saveRuns(runs);
  runForm.reset();
